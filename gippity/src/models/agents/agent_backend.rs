...
impl AgentBackendDeveloper {
    ...
    async fn call_improved_backend_code(&mut self, factsheet: &mut FactSheet) {
        ...
        let msg_context: String = format!(
            "CODE_TEMPLATE: {:?}, PROJECT_DESCRIPTION: {:?}. 
...
impl SpecialFunctions for AgentBackendDeveloper {
    ...
    async fn execute(
        ...
        let (is_crud_required, is_user_login_and_logout): (bool, bool) =
            match &factsheet.project_scope {
                Some(scope) => (scope.is_crud_required, scope.is_user_login_and_logout),
                None => {
                    return Err(Box::new(std::io::Error::new(
                        std::io::ErrorKind::Other,
                        "Must contain project scope before starting on Backend work",
                    )))
                }
            };
            ...
            match &self.attributes.state {
                ...
                AgentState::UnitTesting => {
                    ...
                    PrintCommand::UnitTest.print_agent_message(
                        ...
                        "Backend Unit Testing: ensure safe code...",
                    );
                    ...
                    if build_backend_server.status.success() {
                        ...
                        if self.bug_count > 2 {
                            PrintCommand::Issue.print_agent_message(
                                self.attributes.position.as_str(),
                                "Exiting agent. Too many bugs found in code.",
                            );
                    ...
                    PrintCommand::UnitTest.print_agent_message(
                        self.attributes.position.as_str(),
                        "Backend Unit Testing: Starting server...",
                    );
                    ...
                    PrintCommand::UnitTest.print_agent_message(
                        self.attributes.position.as_str(),
                        "Launching tests on server in 5 seconds...",
                    );
                    ...
                    for endpoint in check_endpoints {
                        ...
                        match check_status_code(&client, &url).await {
                            ...
                            Err(e) => {
                                ...
                                run_backend_server
                                    .kill()
                                    .expect("Failed to kill the backend web server");
...
pub mod tests {
    ...
    async fn develops_and_saves_website_backend() {
        ...
        let contents: String = fs::read_to_string(
            "/Users/shaun/Code/DEVELOPMENT/autogippity/website/backend/src/main.rs",
        )
