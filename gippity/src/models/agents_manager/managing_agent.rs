...
use crate::apis::call_request::call_openai_api;
...
impl ManagingAgent {
    ...
    pub async fn new(usr_req: String) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let project_description_res: Result<String, Box<dyn std::error::Error + Send>> =
            call_openai_api(vec![func_message]).await;
...
pub mod tests {
    ...
    async fn creates_new_managing_agent() {
        ...
        let managing_agent = ManagingAgent::new(usr_request.to_string())
            .await
